...
public class MainActivity extends AppCompatActivity {
    ...
    // Track scroll to avoid SwipeRefresh triggering when scrolling up in feed
    ...
    private void initViews() {
        ...
        // Fix: only allow pull-to-refresh when user pulls DOWN at top of page
        // prevents conflict with scrolling UP in feed
        ...
        // Intercept touch to decide if SwipeRefresh should be enabled
        geckoView.setOnTouchListener((v, event) -> {
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    touchStartY = event.getY();
                    // Disable pull-to-refresh by default; only enable if pulling down
                    swipeRefresh.setEnabled(false);
                    break;
                case MotionEvent.ACTION_MOVE:
                    float deltaY = event.getY() - touchStartY;
                    // Only enable swipe refresh if user is pulling DOWN (positive delta)
                    swipeRefresh.setEnabled(deltaY > SCROLL_THRESHOLD);
                    break;
            ...
            return false; // pass touch to GeckoView
        });
    ...
    private void confirmClearData() {
        new AlertDialog.Builder(this)
            ...
            .setMessage("Clear history and cache?")
